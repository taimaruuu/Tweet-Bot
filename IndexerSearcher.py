...
#Returns array of .txt files to creat documents
def getTxtFile(path):
    files = []
...
def search(searcher, analyzer):
    ...
    query = QueryParser("tweet", analyzer).parse(command)
    scoreDocs = searcher.search(query, 50).scoreDocs #Dont really know how it works figure it out
    print "total matching documents in: " + str((len(scoreDocs)))

    for scoreDoc in scoreDocs: #dont really know what this is either
        doc = searcher.doc(scoreDoc.doc)
        print "@" + doc.get("u_name") + ": " + doc.get("tweet")
    print "\n------------------------------------------------------"


# Create a searcher for the above defined RAMDirectory
searcher = IndexSearcher(DirectoryReader.open(directory)) #dont really know what this is either
